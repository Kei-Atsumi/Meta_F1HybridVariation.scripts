...
levels(summary$direction) <- c("All Observations", "All transgression", "Larger variation", "Smaller variation")

### Plot ###
plot <- ggplot(
  # rename levels of factors to wrap label text
  summary %>%
    within(levels(direction) <- c("All Obser- vations", "All trans- gression", "Larger variation", "Smaller variation")) %>%
...
ggsave(
  ...
  file = "../Analysis/Variation.TS.frequency.svg", height = 3.4, width = 5.1
  )
  ...
  phylmix <- readRDS(
    ...
    sum <- left_join(
      ...
      mutate(
        ...
        direction =
          ifelse(str_detect(factors, "\\+"), "Toward larger variation",
                 ifelse(str_detect(factors, "\\-"), "Toward smaller variation",
                        ifelse(str_detect(factors, "Father_"), "Exceed father trait variation", "Exceed mother trait variation")
                        )
                 )
        )
