...
### Custom function
We have custom functions named : `get_pred()` and `get_reg()`, all of which are used later (see below for their functionality) and the code are included here. 
...
__Study.name__
Including author name(s) and published year

__Source__
Source of phenotypc data in the primary studes

__Cross.ID__
Parental species used in the crossing expreiments. Distinguishes intraspecific populations.

__Gennus__
Studied genus

__sp1.name__ and __sp2.name__: Crossed species. Ordered alphabetically.

__species.pair__: Parental species used in the crossing expreiments. Not distinguish intraspecific populations.

__homologous__: Measured traits are homologous across all crosses. If not, data has been excluded

__trait.type__: Sound or Morphology. Used in regressions bellow as a moderator.

__trait__: Trait name described in primary studies

__Mn.~__: Phenotypic mean of each cross

__SD.~__: SD of phenotype in each cross

__SE.~__: SE of phenotype in each cross

__Parents.~__: Number of parents used in each crossing. Usually not provided by the primary studies.

__N.~__: Number of individuals whose phenotype was measured

__reps.~__: Number of measurements, if replicated (especially in behavioral traits). Not used in the analyses bellow.

__Pheno.divergence__: Phenotypic divergence between parents in the focal trait, which is calculated as absolute value of log response ratio (lnRR) of phenotypic means. Used in regressions bellow as a moderator.
...
__Reciprocal__: Viability of reciprocal hybrids, with two levels - *Viable* or *Inviable*

__Genet.divergence__: Natural logarithm of genetic distance at mtDNA COI region (>450bp).

__Distribution__: Distribution overlap - *Allopatry* or *Overlap*

__Hetero.sex__: Heterogametic sex - *Female* (ZW and Z0) or *Male* (XY and X0)
...
```
Data set created here, `dat.novelty.csv`, is used in investigating [phenotypic variability of novel phenotypes](#phenotypic-variability-of-novel-phenotype).
...
### Check data
Checked data of extremely high/low standard error (SE > 1.2 | 1/SE > 140) and confirmed that those data were correctly imported. 
...
All meta-analytic models included following random effects:
...
Checked data of extremely high/low standard error (SE > 1.2 | 1/SE > 8.5) and confirmed that those data were correctly imported. 
...
```
<br>
Phylogenetic tree used. Tree was created for <i>spLL<sub>M</sub></i>

<div style="margin-bottom:70px;">
</div>

## Factors affect phenotypic novelty
We checked robustness of results by conducting identical analysis for `insect` data (removing vertebrate data). <br>

Model included following random effects:
...
print(metaplot)
  ...
  sum %>% 
    ...
    kable("html", digits = 3, 
          caption = paste("Fixed effects:", k, "data", " ")
            ) %>% 
    kable_styling("striped", position = "left") %>%
    scroll_box(width = "100%", height = "500px") %>%
    print()
  ...
  summary(phylmix$VCV/(1+c2))$statistics %>%
    as.data.frame %>%
    kable("html", digits = 3,
          caption = paste("Random effects:", k, "data", " ")
          ) %>% 
    kable_styling("striped", position = "left") %>%
    print()
...
```
While one of significant moderators (`Reciprocal hybrids`) become no longer significant in `insect` data, effects are qualitatively the same in sign. 
...
for (i in c("Reciprocal", "trait.type")) {
    ...
    sum <- left_join(
      ...
      mutate(
        ...
        direction =
          ifelse(str_detect(factors, "interaction\\+"), "Exceed upper range",
                 "Exceed lower range"
                 ) %>%
          as.factor
        ) %>%
...
```
<br>
Phylogetic tree used in the analyses. Tree was created for <i>spLL<sub>V</sub></i>.
...
```
While significant moderators become no longer significant in `insect` data, effects are qualitatively the same in sign.  
...
dat %>% 
  ...
  kable("html", digits = 1) %>% kable_styling("striped", position = "left")
...
```
<br>
Phylogenetic tree used. Tree was created for <i>sp1</i>, irrespective of the relative trait size and variability compared to the other parent species.
