...
We have custom functions named : `get_pred()`, `get_reg()` and `get_stats()`, all of which are used later (see below for their functionality) and the code are included here. 
...
## Calculate meta-data
`pheno.div.ratio`: Absolute value of log ratio of parents' phenotypic means <br>
`reciprocal`: Viability of reciprocal hybrids, with two levels - `Viable` or `Inviable` <br>
`divergence.COI`: Genetic distance at mtDNA COI region <br>
`distribution`: Distribution overlap - `Allopatry` or `Overlap` <br>
`hetero.sex`: Heterogametic sex - `Female` (ZW and Z0) or `Male` (XY and X0) <br>
...
-- lnRR of hybrids to <i>spSS<sub>M</sub></i>: $\ln\frac{hybrid}{spSS_M}$ <br> 
