...
meta <- read.xlsx(
    "../data/original.data.xlsx", sheet = "Species.level.data"
    ) %>%
...
bind_rows(
  ...
  ### Join with metadata ###
  left_join(
    ., read.xlsx("../data/original.data.xlsx", sheet = "Species.level.data")
    ) %>%
  mutate_at(vars(contains("divergence")), as.numeric) %>%
...
print(metaplot)
  ...
  sum %>% 
    ...
    kable("html", digits = 3, 
          caption = paste("Fixed effects estimates:", k, "data", " ")
            ) %>% 
    kable_styling("striped", position = "left") %>%
    scroll_box(width = "100%", height = "500px") %>%
    print()
...
summary(phylmix$VCV/(1+c2))$statistics %>%
  as.data.frame %>%
  kable("html", digits = 3, caption = "Random effects estimates") %>% 
  kable_styling("striped", position = "left")
...
### Fixed effects output ###
summary(phylmix)$solutions %>% 
  kable("html", caption = "Mean dmismatch value (Fixed effects estimates)") %>% 
  kable_styling("striped", position = "left")
