...
get_fixed.MCMCglmm <- function(x) {
  ...
  x %>%
    ...
    rename("Estimates" = "post.mean", "P" = "pMCMC") %>%
    mutate_at("Estimates", round, 2) %>%
    mutate(significance = ifelse(P < 0.05, "*", "")) %>%
...
<<<<<<< HEAD
    ...
    rename("Estimates" = "post.mean") %>%
    mutate_at("Estimates", round, 2) %>%
    select(group, Factors, Estimates, '95% credible interval', pMCMC, significance) %>%
=======
        sep = "\u2013" # en-dash
        )
      ) %>%
...
>>>>>>> 42222c66d49d042a8e3c2a76d3b4a8c6c19346d1
    kable("html", digits = 3, 
