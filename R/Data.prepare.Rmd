...
LS.mean <- bind_rows(sp1sp2.mean, sp2sp1.mean) %>%
  arrange(ES.ID)


############ export ############
write.csv(
  LS.mean,"../data/dat.mean.csv", quote=F,row.names = F
  )
...
############ combine dataset & sort by ES.ID ############
LS.var <- bind_rows(sp1sp2.var, sp2sp1.var) %>%
  arrange(ES.ID)

############ export ############
write.csv(LS.var,"../data/dat.CV.csv", quote=F,row.names = F)
