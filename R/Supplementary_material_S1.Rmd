...
author: "Keisuke Atsumi, Malgorzata Lagisz & Shinichi Nakagawa"
date: '`r format(Sys.time(), "%y/%m/%d %H:%M")`'
bibliography: pheno_dist_hyb.bib                                    
csl: evolution.csl 
output:
  html_document:
    code_download: true
...
get_pred <- function(model, mod = " ") {
  ...
  if (len != 1) {
    ...
    as.data.frame() %>%
    mutate_at(vars('Estimate':'Praw'), as.numeric) %>%
    mutate(P = ifelse(Praw < 0.001, "<0.001", round(Praw, 3))) %>% 
    mutate(significance = ifelse(Praw < 0.05, "*", "")) %>% 
    mutate_at("P", as.character) %>% 
    select(-Praw)
}
...
get_fixed.MCMCglmm <- function(x) {
  ...
  x %>%
    ...
    mutate(
      ...
      P = ifelse(pMCMC < 0.001, "< 0.001", round(pMCMC, 3))
      ) %>%
...
## Species composition
```{r, results = 'asis', fig.height=5, fig.width=4.8}
...
plot.sum
# ggsave(plot.sum, file = "../Plot/Species_composition.svg", height = 3.4, width = 4.5)
...
```
Data set created here, `dat.mean.csv`, is used in calculating effect sizes for  analyses investigating [phenotypic mean](#meta-analysis-for-phenotypic-mean) and [novelty in phenotypic means](#novel-phenotype-expression).
...
```
Data set created here, `dat.CV.csv`, is used in calculating effect sizes for analyses investigating [phenotypic variation](#meta-analysis-for-phenotypic-variation) and [novelty in phenotypic variabilities](#novel-variability-expression).
...
To quantify phenotypic difference from <i>spSS<sub>M</sub></i> to the other crosses, we calculated log response ratio (lnRR) from phenotypic mean of <i>spSS<sub>M</sub></i> to those of the other crosses (<i>hybLS<sub>M</sub></i> (*spLL* mother × *spSS* father), <i>hybSL<sub>M</sub></i> (*spSS* mother × *spLL* father) and <i>spLL<sub>M</sub></i>).<br>
...
We judged if hybrids exhibit larger or smaller trait size than both parents (see Fig. 3c)
...
```{r, fig.height = 6.5, fig.cap = "Figure S1. Phylogenetic tree used in meta-analysis for phenotypic mean, which was based on tree of the parental species with larger phenotypic mean"}
...
```
<br>
Phylogenetic tree created based on taxonomic tree on [Open Tree of Life](https://tree.opentreeoflife.org/opentree/argus/opentree12.3@ott93302). Non-binary tree was randomly solved. We used packages `rotl`, `ape` and `phytools` on R.
...
row.names(cap) <- c("Alltraits.alltaxon", "Alltraits.insect", "Nonnoveltraits.alltaxon", "Nonnoveltraits.insect")
...
for (tr in c("Alltraits", "Nonnoveltraits")) {
  for (taxon in c("alltaxon", "insect")) {
    ...
    # Rename crossing names
    res.ma$cross <- c("Large × Large", "Large mother ×\nSmall father", "Small mother ×\nLarge father")
    dat.plot$cross <- factor(dat.plot$cross, levels = c("spL", "hybLS", "hybSL"))
    levels(dat.plot$cross) <- c("Large × Large", "Large mother ×\nSmall father", "Small mother ×\nLarge father")
    # Bind result of all crosses at each dataset
    assign(paste("res.ma", tr, taxon, sep = "."), res.ma)
    ...
    plot.metamean <- ggplot(data = res.ma, aes(x = tanh(Estimate), y = cross)) +
      scale_y_discrete(
        limits = c("Small mother ×\nLarge father", "Large mother ×\nSmall father", "Large × Large"),
        expand = c(0.05, 0.8)
        ) +
      ...
      ## I^2 ###
      annotate(
        "text", x = -1, y = c(1.41, 2.41, 3.26),
        label = "italic(I^2)",
        parse = TRUE, hjust = "left", size = 4
      ) +
      annotate(
        "text", x = -0.9, y = c(1.4, 2.4, 3.25),
        label = paste("=", res.ma$I2), hjust = "left", size = 4
      ) +
      ## N ###
      annotate(
        "text", x = 0.75, y = 3.7,
        label = paste("italic(N)==", length(dat.plot$ES.ID)/3), 
        parse = TRUE, hjust = "right", size = 4
      ) + 
      annotate(
        "text", x = 1, y = 3.7,
        label = "each", hjust = "right", size = 4
      ) + 
      # Labels
      labs(
        y = "", 
        x = "Hyperbolic tangent of\nlnRR from small non-hybrid",
        size = "Precision"
      ) +
      orchardtheme
    
    if (taxon == "alltaxon") {
      assign(paste0("orchardplot_lnRR_", tr), plot.metamean)
    }
    ...
    # ggsave(plot = plot.metamean, file = paste("../Analysis/mean.general.metamean", tr, taxon, "svg", sep = "."),
    #   height = 4, width = 5
    #   )
  }
}
...
bind_rows(
  ...
  as.data.frame() %>%
  select("Dataset", "cross", "Estimate", contains("erCI"), contains("PR"), "P", "significance", "N", contains("I2")) %>% 
...
We quantified relative phenotypic variability of hybrids and the parent with large variability (<i>spLL<sub>V</sub></i>) to the parent with small variability (<i>spSS<sub>V</sub></i>), by using [lnCVR](https://besjournals.onlinelibrary.wiley.com/doi/full/10.1111/2041-210X.12309) [@Nakagawa2015]. We controlled mean-variance relationship by using Coefficient of Variation (CV) as the proxy of phenotypic variability. <br>
Effecct size was calculated by using `escalc` function in `metafor` package 
...
We judged if hybrids have larger or smaller CV in trait size than both parents (see Fig. 3c)
...
```{r, fig.height = 6.5, fig.cap = "Figure S2. Phylogenetic tree used in meta-analysis for phenotypic variability (coefficient of variation, CV), which was based on tree of the parental species with larger CV in trait size"}
...
We compared midparent-value of phenotypic variability (CV) and hybrids' phenotypic variability by calculating log ratio of phenotypic variability (lnCVR) of midparent-value and hybrids to <i>spSS<sub>V</sub></i>. <br> 
...
## Univarate meta-analysis for each cross
Quantify and visualise overall trend of phenotypic variability (CV) of hybrids compared to parents <br>
Meta-analysis was repeated for subset data - including only non-novel phenotypes, only insects, and non-novel phenotypes in insects <br>
Meta-analysis was conducted by using `rma.mv` function in `metafor` package <br>
...
for (tr in c("Alltraits", "Nonnoveltraits")) {
  for (taxon in c("alltaxon", "insect")) {
    ...
    plot.metamean <- ggplot(data = res.ma, aes(x = tanh(Estimate), y = cross)) +
      ...
      labs(
        y = "", 
        x = "Hyperbolic tangent of\nlnCVR from small non-hybrid",
        size = "Precision"
      ) +
      orchardtheme

    if (taxon == "alltaxon") {
      assign(paste0("orchardplot_lnCVR_", tr), plot.metamean)
    }
...
summary$direction <- ordered(summary$direction, levels = c("All Observations", "Novel means", "+", "-"))
# Change names of TS category to more intuitive name
levels(summary$direction) <- c("All Observations", "Novel means", "Exceed upper range", "Exceed lower range")

## Plot ###
plot <- ggplot(
  # rename levels of factors to wrap label text
  summary %>%
    within(levels(direction) <- c("All Obser- vations", "Novel means", "Exceed upper range", "Exceed lower range")) %>%
    within(levels(metrics) <- c("Study", "Species pair", "Observations")), 
  aes(x = "", y = N, fill = taxa)) +
  geom_bar(width = 1, stat = "identity") +
  scale_fill_brewer(palette = "Paired") +
  facet_grid(
...
# ggsave(plot, file = "../Plot/part_novelmeanfrequency.svg", height = 3.5, width = 5.2)
...
print(metaplot)
  ...
  #+++++++++++++++++++++++++#
  sum %>% 
    select(Group, Factors, Estimates, '95% credible interval', P, significance) %>%
...
for (i in c("Genet.divergence", "Pheno.divergence")) {
  ...
  #   file = paste("../Plot/part_novelmeanscontinuous", i, "svg", sep = "."),
...
# Change names of TS category to more intuitive name
levels(summary$direction) <- c("All Observations", "Increase novelty", "Larger variabilities", "Smaller variabilities")

## Plot ###
plot <- ggplot(
  # rename levels of factors to wrap label text
  summary %>%
    within(levels(direction) <- c("All Obser- vations", "Novel varia- bilities", "Larger varia- bilities", "Smaller varia- bilities")) %>%
    within(levels(metrics) <- c("Study", "Species pair", "Observations")), 
  aes(x = "", y = N, fill = taxa)) +
  geom_bar(width = 1, stat = "identity") +
  scale_fill_brewer(palette = "Paired") +
...
# ggsave(plot, file = "../Plot/part_novelvarfrequency.svg", height = 3.5, width = 5.2)
...
for (i in c("Genet.divergence", "Pheno.divergence")) {
  ...
  # ggsave(
  #   plot = plot + theme(axis.title  = element_blank()),
  #   file = paste("../Plot/part_novelvarscontinuous", i, "svg", sep = "."),
  #   height = 1.7, width = 1.6
  # )
...
* Smaller: Smaller variabilities than of both parents

* Intermediate: intermediate between parents' variability 

* Larger: Larger variabilities than of both parents
...
```{r, fig.height = 6.5, fig.cap = "Figure S3. Phylogenetic tree used, which was based on tree of the parental species within a cross which name comes earlier in alphabetical order"}
...
get_fixed.MCMCglmm(summary(phylmix)$solutions) %>%
  dplyr::select("Factors", "Estimates", "95% credible interval", "P") %>%
...
To quantify trait mosaicism in each crossing, we calculated d<sub>mismatch</sub>  following [Thompson et al.](https://www.biorxiv.org/content/10.1101/818658v1.full#disp-formula-1) -@Thompson2021. d<sub>mismatch</sub> was calculated as follows:
...
```
Even data included quite large d<sub>mismatch</sub>, overall it did not differ from 0. Yet, the estimate (β  = 4.41, CI = -3.05 – 10.29) is still much larger than  previously reported mean d<sub>mismatch</sub> 0.6 [@Thompson2021]. This is presumably because novelty in phenotypic means, which should increase d<sub>mismatch</sub>, is much more frequent in our data compared to [Thompson et al.](https://www.biorxiv.org/content/10.1101/818658v1.full#disp-formula-1) -@Thompson2021.

<div style="margin-bottom:70px;">
</div>

## Factors affecting  d<sub>mismatch</sub>
...
```
None of species level moderators did not influence d<sub>mismatch</sub>, as [Thompson et al.](https://www.biorxiv.org/content/10.1101/818658v1.full#disp-formula-1)-@Thompson2021 showed. 
...
To investigate paternal effect in hybrids using difference in variance between reciprocal cross, it's better to allign species with larger phenotypic variance (<i>spLL<sub>V2</sub></i>) and smaller CV (<i>spSS<sub>V2</sub></i>) in each traits. Name of hybrids will be changed according to change in names of parental species (<i>hybridLS<sub>V2</sub></i> & <i>hybridSL<sub>V2</sub></i>).  
...
```
<!-- Data set created here, `dat.var.csv`, is used in calculating effect sizes for analyses investigating [phenotypic variation](#meta-analysis-for-phenotypic-variation) and [novelty in phenotypic variabilities](#novel-variability-expression). -->


## Effect size calculation

We quantified relative phenotypic variability of hybrids and the parent with large variability (<i>spLL<sub>V2</sub></i>) to the parent with small variability (<i>spSS<sub>V2</sub></i>), by using [lnVR](https://besjournals.onlinelibrary.wiley.com/doi/full/10.1111/2041-210X.12309). **We compared variance itself**, and did not controll mean-variance relationship. <br>
Effecct size was calculated by using `escalc` function in `metafor` package 
...
## Dominance in phenotypic variability (variance)

We compared midparent-value of phenotypic variability and hybrids' phenotypic variability by calculating log ratio of phenotypic variability (lnVR) of midparent-value and hybrids to <i>spSS<sub>V</sub></i>. <br> 
...
## Crossing direction effect in phenotypic variability  (variance)

We statistically compared lnVR from <i>spSS<sub>M</sub></i> to the other crosses through the formal meta-regression by using `rma.mv` function of R package `metafor`. <br> 
Here we asked if lnVR of <i>hybSL<sub>V</sub></i> was smaller/larger than that of <i>hybLS<sub>V</sub></i>. Smaller lnVR of <i>hybSL<sub>V</sub></i> indicates maternal inheritance in phenotypic variability <br>
...
<!-- orchards = ((orchardplot_lnRR_Alltraits* -->
<!--   theme(axis.text.x = element_blank(), axis.title.x = element_blank()) / -->
<!--   orchardplot_lnRR_Nonnoveltraits) |  -->
<!--   (orchardplot_lnCVR_Alltraits* -->
<!--   theme(axis.text = element_blank(), axis.title.x = element_blank()) / -->
<!--   orchardplot_lnCVR_Nonnoveltraits)*  -->
<!--   theme(axis.text.y = element_blank())) + -->
<!--   plot_annotation(tag_levels = 'A') -->
<!-- orchards -->
<!-- # ggsave("../Plot/orchards.svg", orchards, height = 7, width = 8) -->
