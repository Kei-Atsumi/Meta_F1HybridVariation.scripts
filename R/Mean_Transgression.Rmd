...
#   inspect(taxa, ott_id = taxa$ott_id[taxa$number_matches != 1][1])[1, "ott_id"]
# inspect(taxa, ott_id = taxa$ott_id[taxa$number_matches != 1][2])
# taxa[taxa$number_matches != 1, ][2] <-
#   inspect(taxa, ott_id = taxa$ott_id[taxa$number_matches != 1][2])[1, "ott_id"]
# inspect(taxa, ott_id = taxa$ott_id[taxa$number_matches != 1][3])
# taxa[taxa$number_matches != 1, ][3] <-
#   inspect(taxa, ott_id = taxa$ott_id[taxa$number_matches != 1][3])[1, "ott_id"]
# inspect(taxa, ott_id = taxa$ott_id[taxa$number_matches != 1][4])
# taxa[taxa$number_matches != 1, ][4] <-
#   inspect(taxa, ott_id = taxa$ott_id[taxa$number_matches != 1][4])[1, "ott_id"]
# 
...
ggsave(
  plot = plot, 
  file = "../Analysis/mean.TS.frequency.svg", height = 3.4, width = 5.1
  )
  ...
  for (k in c("alltaxon", "insect")) {
    ...
    sum <- left_join(
      ...
      mutate(
        ...
        direction =
          ifelse(str_detect(factors, "\\+"), "Toward larger mean",
                 ifelse(str_detect(factors, "\\-"), "Toward smaller mean",
