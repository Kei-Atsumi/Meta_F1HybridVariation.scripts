...
prior <- list(
  B=list(
    ...
    V = gelman.prior(
      ~ direction*divergence.COI + direction*sq.divergence.COI + direction*pheno.div.ratio + direction*parental + direction*trait.type + direction*distribution + direction*reciprocal + parental*hetero.sex, 
...
metaplot <- ggplot(sum, aes(y = post.mean, x = factors)) +
  ...
  scale_colour_manual(values = c("grey50", "black"), aesthetics = "colour") +
  geom_point(size = 1.5, aes(colour=significance))+
  scale_fill_manual(values = c("grey50", "black"), aesthetics = "colour") +
  scale_x_discrete(limits = rev(sum$factors))+ # reverse x-axis
  coord_flip() +
  xlab("") + ylab("estimate with CI") +
  theme_bw() +
  theme(
    panel.grid = element_blank(),
