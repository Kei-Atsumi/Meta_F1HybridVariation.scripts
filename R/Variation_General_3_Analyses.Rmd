  ...
  for (k in c("alltaxon", "insect")) {
    ...
    print(paste("hybLS is larger than midparent in",
        round(
          ...
          2),
        "percent | P =",
        round(midparent.compare$pval[2],4)
        ))
    print(paste("hybSL is larger than midparent in",
        round(
          ...
          2),
        "percent | P =",
        round(midparent.compare$pval[3],4)
        ))
  ...
  for (k in c("alltaxon", "insect")) {
    for (h in c("spL", "hybLS", "hybSL")) {
      ...
      phyl.random <- rma.mv(
        ...
        random = list(~1 | spL.name, ~1 | Study.ID, ~1 | Cross.ID, ~1 | ES.ID),
        R = list(spL.name = phylo_cor),
        control=list(maxiter=2000, stepadj=0.5, optimizer="optim", optmethod="Nelder-Mead"),
        )
      ...
      assign(
        ...
        get_pred(phyl.random) %>%
          as.data.frame() %>%
          mutate(N = length(dat$ES.ID)) %>%
          mutate(
            I2 = I2(phyl.random)["I2_total"] %>%
              round(3),
    ...
    plot.metamean <- ggplot(
      ...
      annotate(
        ...
        label = paste("italic(N)==", res.ma$N, "| italic(I^2)==", res.ma$I2), 
