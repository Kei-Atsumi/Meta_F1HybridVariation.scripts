...
for (j in c("withTS", "withoutTS")) {
  for (k in c("all", "insect")) {
    ...
    # saveRDS(
    #   midparent.compare, 
    #   file = paste("../Analysis/Variation.midparent.compare", j, "obj", sep = ".")
    #   )
  
    ### Load model ###
    midparent.compare <- readRDS(paste("../Analysis/Variation.midparent.compare", j, "obj", sep = "."))
    
    print(j)    
    print(summary(midparent.compare))
  
    print(paste("hybLS is larger than midparent in",
        round(
          (exp(midparent.compare$beta[1]+midparent.compare$beta[2]) -
             exp(midparent.compare$beta[1])
           )*100,
          2),
        "percent"))
    print(paste("hybSL is larger than midparent in",
        round(
          (exp(midparent.compare$beta[1]+midparent.compare$beta[3]) -
             exp(midparent.compare$beta[1])
           )*100,
          2),
        "percent"))
    ...
    # saveRDS(
    #   midparent.random, 
    #   file = paste("../Analysis/Variation.midparent", j, "obj", sep = ".")
    #   )
  
    
    ### Load model ###
    midparent.random <- readRDS(paste("../Analysis/Variation.midparent", j, "obj", sep = "."))
  ...
  for (h in c("spL", "hybLS", "hybSL")) {
    ...
    assign(
      ...
      get_pred(phyl.random) %>%
        as.data.frame() %>%
        mutate(N = length(split.data$ES.ID)) %>%
        mutate(
...
for (j in c("withTS", "withoutTS")) {
  ...
  # saveRDS(
  #   phyl.random.spL,
  #   file = paste("../Analysis/Variation.compare.from.spL", j, "obj", sep = ".")
  #   )
  # saveRDS(
  #   phyl.random.hybLS,
  #   file = paste("../Analysis/Variation.compare.from.hybLS", j, "obj", sep = ".")
  #   )


  ### Load model ###
  phyl.random.spL <- readRDS(paste("../Analysis/Variation.compare.from.spL", j, "obj", sep = "."))
  phyl.random.hybLS <- readRDS(paste("../Analysis/Variation.compare.from.hybLS", j, "obj", sep = "."))

  print(j)
  print(summary(phyl.random.hybLS))
  print(paste("hybSL is larger than hybLS in",
  round(
...
for (j in c("withTS", "withoutTS")) {
  ...
  #   random = list(~1 | spL.name, ~1 | Study.ID, ~1 | Cross.ID),
  #   R = list(spL.name = phylo_cor),
  #   mods = ~ cross*hetero.sex + pheno.div.ratio
  #   )
  # saveRDS(
  #   phyl.random,
  #   file = paste("../Analysis/Variation.metareg", j, "obj", sep = ".")
  #   )
  
  ### Load model ###
  phyl.random <- readRDS(paste("../Analysis/Variation.metareg", j, "obj", sep = "."))
