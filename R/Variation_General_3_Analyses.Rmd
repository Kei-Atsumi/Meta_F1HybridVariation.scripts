...
for (j in c("withTS", "withoutTS")) {
  for (k in c("alltaxon", "insect")) {
    ...
    get_reg(midparent.compare) %>%
      ...
      within('Fixed effects' <- c("", "midparent (intrcpt)", "hybLS", "spSL")) %>%
      print()
      ...
      assign(
        ...
        get_pred(phyl.random) %>%
          ...
          mutate(
            ...
            data.type = paste(j,k) # indicate dataset without TS
            ) %>%
          mutate(metaunit =  str_c(data.type, cross, sep = "_"))
          )
...
for (j in c("withTS", "withoutTS")) {
  for (k in c("alltaxon", "insect")) {
    ...
    # saveRDS(
    #   phyl.random,
    #   file = paste("../Analysis/Variation.metareg", j, k, "obj", sep = ".")
    #   )
    
    ### Load model ###
    phyl.random <- readRDS(paste("../Analysis/Variation.metareg", j, k, "obj", sep = "."))
    ### Output result ###
