...
for (j in c("withTS", "withoutTS")) {
  for (i in c("lnRR")) {
    ...
    print("metadata for species pair")
    print(
      hybrids %>%
        distinct(species.pair, .keep_all = TRUE) %>%
        select(hetero.sex, distribution) %>%
        drop_na() %>%
        count(hetero.sex, distribution)
    )
...
for (j in c("withTS", "withoutTS")) {
  for (i in c("lnRR")) {
    ...
    phyl.random <- rma.mv(
      ...
      random = list(~1 | spL.name, ~1 | Study.ID, ~1 | Cross.ID),
      R = list(spL.name = phylo_cor),
      mods = ~ divergence.COI + sq.divergence.COI + pheno.div.ratio + cross.sex -1
        # + cross*divergence.COI + cross*pheno.div.ratio
      )
