...
for (j in c("withTS", "withoutTS")) {
  for (k in c("alltaxon", "insect")) {
    ...
    #   random = list(~1 | spL.name, ~1 | Study.ID, ~1 | Cross.ID),
    #   R = list(spL.name = phylo_cor),
    #   mods = ~ pheno.div.ratio + cross*hetero.sex
    #     # + cross*divergence.COI + cross*pheno.div.ratio
    #   )
    # saveRDS(
    #   phyl.random,
    #   file = paste("../Analysis/Mean.metareg", j, k, "obj", sep = ".")
    #   )
    
    ### Load model ###
    phyl.random <- readRDS(paste("../Analysis/Mean.metareg", j, k, "obj", sep = "."))
...
```{r maternal vs paternal categorical}
