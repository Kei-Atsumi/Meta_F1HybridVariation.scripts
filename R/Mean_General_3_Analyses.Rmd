...
pacman::p_load(
  ...
  , plotly      # interactive plots using ggplot
)
...
get_pred <- function(model, mod = " ") {
  I2s <- i2_ml(model) %>%
    as.data.frame() 
  I2percentage <- round(I2s/I2s[1,1]*100, 1) # I2s[1,1] = I2 total
  ...
  if (len != 1) {
    ...
    'I2[total]' = 
      c(round(I2s[1,1], 3), rep("", length(model$beta)-1)),
    '%I2[species with phylogeny]' = 
      c(paste0(I2percentage["I2_spL.name",1],"%"), rep("", length(model$beta)-1)),
    '%I2[study]' =
      c(paste0(I2percentage["I2_Study.ID",1],"%"), rep("", length(model$beta)-1)),
    '%I2[crossed strain]' =
      c(paste0(I2percentage["I2_Cross.ID",1],"%"), rep("", length(model$beta)-1)),
    '%I2[residual]' = 
      c(paste0(I2percentage["I2_ES.ID",1],"%"), rep("", length(model$beta)-1)),
    )  %>%
    as.data.frame() %>%
    mutate_at(vars('Estimate':'I2[total]'), as.numeric) %>%
...
get_reg <- function(model, mod = " ") {
  ...
  table <- tibble(
    ...
    #   c(model$sigma2[4], rep("", length(model$beta))),
    'I2[total]' = 
      c(round(I2s[1,1], 3), rep("", length(model$beta))),
    '%I2[species with phylogeny]' = 
      c(paste0(I2percentage["I2_spL.name",1],"%"), rep("", length(model$beta))),
    '%I2[study]' =
      c(paste0(I2percentage["I2_Study.ID",1],"%"), rep("", length(model$beta))),
    '%I2[crossed strain]' =
      c(paste0(I2percentage["I2_Cross.ID",1],"%"), rep("", length(model$beta))),
    '%I2[residual]' = 
      c(paste0(I2percentage["I2_ES.ID",1],"%"), rep("", length(model$beta))),
    )  %>%
    as.data.frame() %>%
    mutate_at(vars('Estimate':'I2[total]'), as.numeric) %>%
    mutate(significance = ifelse(P < 0.05, "*", ""))
...
Comparing the differences between parentals and hybrids  
Indicates that maternal inheritance become stronger in male-heterogametic species, but, dataset is heavily biased toward male-heterogametic species so this effect is still uncertain  
Included the analysis accomodating potential publication bias by including SE of effect size as a moderator
...
for (l in c("normal", "sensitivity")) {
  for (j in c("withTS", "withoutTS")) {
    for (k in c("alltaxon", "insect")) {
      ...
      # saveRDS(
      #   phyl.random,
      #   file = paste("../Analysis/Mean.metareg", j, k, l, "obj", sep = ".")
      #   )
      
      ### Load model ###
      phyl.random <- readRDS(paste("../Analysis/Mean.metareg", j, k, l, "obj", sep = "."))
  
  
      ### Output result ###
      reg <- get_stats(phyl.random) %>%
        mutate_at("Fixed_effects", as.factor)
      plot <- ggplot(reg, aes(y = Estimate, x = Fixed_effects)) +
        geom_errorbar(
...
# Assesing maternal inheritance part 2
Comparing the differences between parentals and hybrids  
Model did not converge when data is restricted to without TS

```{r maternal vs paternal categorical, results='asis'}
...
for (j in c("withTS")) {
  for (k in c("alltaxon", "insect")) {
    ...
    if (k == "alltaxon") {
      dat <- with.without.TS %>% 
        filter(data.type == j, cross != "spL") %>%
        mutate(cross.sex = str_c(hetero.sex, cross, sep = "_"))
    } else {
      dat <- with.without.TS %>% 
        filter(data.type == j, insect == k, cross != "spL") %>%
        mutate(cross.sex = str_c(hetero.sex, cross, sep = "_"))
    ...
    phyl.random <- rma.mv(
      ...
      random = list(~1 | spL.name, ~1 | Study.ID, ~1 | Cross.ID, ~1 | ES.ID),
      R = list(spL.name = phylo_cor),
      mods = ~ pheno.div.ratio + cross.sex -1
      )
    ...
    get_reg(phyl.random) %>%
      ...
      # Show dataset name
      within(Dataset <- c(paste(j,k), rep("", length(.$Estimate)-1))) %>%
      kable("html", digits = 3, caption = paste(j,k)) %>% 
      kable_styling("striped", position = "left") %>%
      print()
...
for (j in c("withTS", "withoutTS")) {
  for (k in c("alltaxon", "insect")) {
    ...
    # saveRDS(
    #   midparent.random,
    #   file = paste("../Analysis/Mean.PET-PEESE.midparent", j, k, "obj", sep = ".")
    #   )

    ### Load model ###
    midparent.random <- readRDS(paste("../Analysis/Mean.PET-PEESE.midparent", j, k, "obj", sep = "."))
    # creating summary table of results
    ...
    for (h in c("spL", "hybLS", "hybSL")) {
      ...
      #   )
      # saveRDS(phyl.random,
      #   file = paste("../Analysis/Mean.PET-PEESE", j, k, h, "obj", sep = "."))

      
      ### Load model ###
      phyl.random <- readRDS(paste("../Analysis/Mean.PET-PEESE", j, k, h, "obj", sep = "."))
