...
# ##########################################
# 
# mod.select <- dredge(phylmix, rank="DIC")
# summary(model.sel(mod.select))
...
sum.categorical <- bind_rows(sum.reciprocal, sum.parental, sum.hetero.sex) %>%
  as.data.frame() %>%
...
ggsave(
    ...
    file = "../Analysis/mean.TS.categorical.svg", 
    height = 6, width = 5
    )
